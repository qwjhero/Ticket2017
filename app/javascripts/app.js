...
function initializeConference() {
	myConferenceInstance = Conference.deployed();
	//myConferenceInstance = Confenrence.at(confenrence.delpoyed_address);
	//$("#confAddress").html(myConferenceInstance);
...
function lookup(Address_lookup){
	myConferenceInstance.registrantsPaid.call(Address_lookup).then(
		function(valuePaid) {
			$("#ticketPrice_lookup").html(web3.fromWei(valuePaid.toNumber(),'ether'));
			var msgResult;
			if (valuePaid.toNumber()> 0) {
				msgResult = "Already Purchase";
			} else {
				msgResult = "No Purchase";
			}
			$("#lookupResult").html(msgResult);
		});
